...
class Auth {
  ...
  static Future<void> createUserWithEmailPass(
    ...
    try {
      ...
      ShowSnackBar.showSnackBar(
          context, 'Success Registration.', Colors.orange[400]!);
    } on FirebaseAuthException catch (ex) {
      if (ex.code == 'weak-password') {
        ShowSnackBar.showSnackBar(
            context, 'can\'t use a weak password.', Colors.red[300]!);
      } else if (ex.code == 'email-already-in-use') {
        ShowSnackBar.showSnackBar(
            context, 'can\'t use an existing email.', Colors.red[300]!);
      }
    } catch (ex) {
      ShowSnackBar.showSnackBar(
  ...
  static Future<void> signInUserWithEmailPass(
      BuildContext context, String email, String pass) async {
    try {
      ...
      if (ex.code == 'user-not-found') {
        ShowSnackBar.showSnackBar(
            context, 'No user found for that email.', Colors.red[300]!);
      } else if (ex.code == 'wrong-password') {
        ShowSnackBar.showSnackBar(context,
            'Wrong password provided for that user.', Colors.red[300]!);
      }
    } catch (ex) {
      ShowSnackBar.showSnackBar(
          context, 'error exist. please try again later.', Colors.red[300]!);
